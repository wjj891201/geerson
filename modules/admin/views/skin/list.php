            ...
            <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
                ...
                <?php foreach ($all as $key => $vo): ?>
                    <tr>
                        ...
                        <td align="center">
                            <?php if ($vo['isclass'] != 1): ?>
                                <a class="setedit" href="javascript:void(0);" onclick="layer.confirm('确认启用',{icon:3,fix:true},function(index){layer.close(index);location.href='<?= Url::to(['skin/open', 'skid' => $vo['skid']]) ?>';});">启用</a>
                            <?php else: ?>
                                <a class="setedit2" href="javascript:void(0);">已启用</a>
                            <?php endif; ?>
                            <?php if ($vo['lockin'] != 1): ?>
                                <a class="setedit" href="javascript:void(0);" onclick="layer.confirm('确认删除',{icon:3,fix:true},function(index){layer.close(index);location.href='<?= Url::to(['skin/del', 'skid' => $vo['skid']]) ?>';});">删除</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
