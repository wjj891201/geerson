...
class Type extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
                ...
                [['upid', 'typename_en', 'typepic', 'content', 'headtitle', 'keywords', 'description', 'styleid', 'ismenu', 'isorderby', 'ordertype', 'ispart', 'isline', 'typeurl', 'gotoline'], 'safe']
        ];
    }
    ...
    {
        $top = self::find()->select(['tid', 'typename', 'typename_en'])->where($where)->asArray()->orderBy(['pid' => SORT_ASC])->all();
...
//    }
    ...
    //移动端分类链接
    public static function getMobileUrl($tid)
    {
        $info = self::getone(['tid' => $tid]);
        if (!empty($info))
        {
            ...
            {
                case 1://频道
                    $url = Yii::$app->urlManager->createUrl(['mobile/article/channel', 'tid' => $info['tid']]);
                    break;
                case 2://列表
                    $url = Yii::$app->urlManager->createUrl(['mobile/article/list', 'tid' => $info['tid']]);
                    break;
                case 4://单页
                    $url = Yii::$app->urlManager->createUrl(['mobile/single/detail', 'tid' => $info['tid']]);
                    break;
                case 3://跳转
                    if ($info['isline'] == 0)
                    {
                        ...
                        {
                            case 1://频道
                                $url = Yii::$app->urlManager->createUrl(['mobile/article/channel', 'tid' => $sec['tid']]);
                                break;
                            case 2://列表
                                $url = Yii::$app->urlManager->createUrl(['mobile/article/list', 'tid' => $sec['tid']]);
                                break;
                            case 4://单页
                                $url = Yii::$app->urlManager->createUrl(['mobile/single/detail', 'tid' => $sec['tid']]);
                                break;
                        }
                    }
                    else
